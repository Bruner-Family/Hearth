...
type ColumnDef = {
   ...
   *  Fixed widths sum (with the Name min-width) to under the ~736px content box
   *  at the 768px breakpoint, so nothing clips; Name's flex-1 absorbs slack on
   *  wider screens. */
  className: string;
...
const COLUMNS: ColumnDef[] = [
  { key: "name", label: "Name", className: "flex-1 min-w-[9rem]", cell: (i) => i.name },
  {
    key: "category",
    label: "Category",
    className: "w-32",
...
function sortHint(active: boolean, dir: SortDir): string {
  if (!active) return "";
  return `, sorted ${dir === "asc" ? "ascending" : "descending"}`;
}
          ...
          {COLUMNS.map((col) => {
            ...
            return (
              <Pressable
                ...
                accessibilityLabel={`Sort by ${col.label}${sortHint(active, sort.dir)}`}
                className={`flex-row items-center gap-1 px-3 py-2.5 active:opacity-70 ${col.className}`}
                onPress={() => onSortChange(col.key)}
              >
                <Text
                  className="text-xs font-semibold uppercase tracking-wider text-ink-dim"
                  numberOfLines={1}
                >
                  {col.label}
                </Text>
